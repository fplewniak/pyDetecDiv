...
class ProjectSelector(GenericSelector):
    def __init__(self, tag='project_selector', source=0, **kwargs):
        ...
        super().__init__(tag, 'Select project', list_projects(), source=f'{source}_combo',
                         callback=self.select, **kwargs)
        self.show_hide()

    def select(self, sender, app_data, user_data):
        registry.close_project().set_project(app_data)
...
class RoiSelector(GenericSelector):
    ...
    def select(self, sender, app_data, user_data):
        roi = registry.project.get_named_object('ROI', app_data)
        dpg.set_value('info_text', roi.info)
