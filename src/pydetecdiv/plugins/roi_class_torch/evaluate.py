...
def evaluate_metrics(model, data_loader, loss_fn, device):
    model.eval()
    running_loss = 0.0
...
def get_pred_gt(model, data_loader, device):
    model.eval()
...
def evaluate_model(model, checkpoint_filepath, class_names, data_loader, seqlen, device):
    predictions, ground_truth = get_pred_gt(model, data_loader, device)

    model = torch.jit.load(checkpoint_filepath)
    best_predictions, best_gt = get_pred_gt(model, data_loader, device)
