...
def evaluate_model(model, checkpoint_filepath, class_names, data_loader, seqlen, device):
    ground_truth = [label.argmax(axis=0) for batch in [y for x, y in data_loader] for label in batch]
    model.eval()
    if seqlen == 0:
        ...
        print(f'{datetime.now().strftime("%H:%M:%S")}: Prediction on test dataset for last model')
        predictions = [label.cpu()[0] for batch in [model(img.to(device)).argmax(axis=2) for img, target in data_loader] for
                       label in batch]

        print(f'{datetime.now().strftime("%H:%M:%S")}: Prediction on test dataset for best model')
        model = torch.jit.load(checkpoint_filepath)
        best_predictions = [label.cpu()[0] for batch in [model(img.to(device)).argmax(axis=2) for img, target in data_loader]
                            for label in batch]
