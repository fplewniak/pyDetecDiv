...
def create_model():
    efficientnet = keras.applications.EfficientNetV2S(input_shape=(60,60,3), weights='imagenet', include_top=False)

    input = keras.Input(shape=(None,60,60,3))
    fold_out, fold_miniBatchSize = SequenceFoldingLayer((60,60,3))(input)

    central_block = efficientnet(fold_out, training=True)
    ...
    unfold = SequenceUnfoldingLayer((1,1,1280))(plugout, fold_miniBatchSize)
