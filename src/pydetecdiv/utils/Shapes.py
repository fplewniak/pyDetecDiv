...
class Box:
    ...
    def __init__(self, top_left=None, bottom_right=None, shape=None):
        """
        If shape is not specified, then it is determined from the coordinates of both corners. If only shape is provided
        the corner coordinates are computed
        :param top_left: top left corner of the box
        :param bottom_right: bottom right corner of the box
        :param shape: size dimensions of the box
    ...
    @top_left.setter
    def top_left(self, top_left: tuple = None):
        if top_left is None:
            self._top_left = (0, 0)
        else:
    ...
    @bottom_right.setter
    def bottom_right(self, bottom_right: tuple = None):
        if bottom_right is None:
            self._bottom_right = (self._shape[0] - 1, self._shape[1] - 1)
        else:
    ...
    def shape(self):
        """
        shape or size dimensions of the box, i.e. width and height
    ...
    def shape(self, shape: tuple = None):
        if shape is None:
            self._shape = (self.width, self.height)
        else:
            self._shape = shape
    ...
    def lies_in(self, other):
        ...
        :rtype: bool
        """
        return (self.top_left[0] >= other.top_left[0] and self.bottom_right[0] <= other.bottom_right[0]) \
               and (self.top_left[1] >= other.top_left[1] and self.bottom_right[1] <= other.bottom_right[1])
